...
(setq borg-rewrite-urls-alist
      '(("git@github.com:"   .  "https://github.com/")
        ...
        ("git@codeberg.org:" .  "https://codeberg.org/")
        ("git@git.sr.ht:"    .  "https://git.sr.ht/")))
...
(use-package mason
  ...
  (setq my/mason-list
        '(
          ...
          c3-lsp
          vim-language-server
          typescript-language-server oxlint oxfmt
          ...
          jq yq))
  (mason-setup)
  (defun my/mason-install ()
    (interactive)
    (dolist (pkg my/mason-list)
...
(use-package lin
  ...
  (appendq! lin-mode-hooks
            '(prog-mode-hook text-mode-hook rg-mode-hook benchmark-init/tabulated-mode-hook))
...
(use-package nerd-icons-dired
  ...
  :init
  (setq nerd-icons-dired-icon-size 0.8))
...
(use-package dired-preview
  :init
  (setopt dired-preview-delay 0.2)
  (setopt dired-preview-kill-buffers-method (cons 'buffer-number 30)))
#+end_src
...
(use-package orderless
  ...
  (orderless-define-completion-style
    ...
    (orderless-matching-styles '(orderless-literal-prefix orderless-literal orderless-flex)))

  (setq completion-category-overrides
        '((file     (styles partial-completion))
          (command  (styles my/orderless-vertico))
          (variable (styles my/orderless-vertico))
          (symbol   (styles my/orderless-vertico))))
...
#+begin_src emacs-lisp :lexical t :tangle no
(use-package corfu
  :hook (corfu-mode-hook . corfu-setup)
  :bind
  (:map corfu-mode-map
  ...
  (setq corfu-popupinfo-delay '(nil . 0.1))
  (global-corfu-mode)
  (defun corfu-setup ()
  ...
  (use-package cape
    ...
    :config
    (setq completion-at-point-functions (list 'cape-dabbrev 'cape-file)))
  )
#+end_src
  ...
  (use-package consult-dir
    ...
    (defvar consult-dir--source-zlua
      `(:name     "Zlua Dir"
        ...
        :enabled  ,(lambda () (and (getenv "ZLUA_SCRIPT") (executable-find "lua")))
        :items
        ,(lambda ()
           (nreverse (mapcar
...
(use-package mini-echo
  ...
  (setq mini-echo-persistent-rule
        '(:long ("meow" "shrink-path" "vcs" "buffer-position"
                 "buffer-size" "remote-host" "flymake" "mise" "envrc")
...
(setq modus-themes-common-palette-overrides
      '((fringe unspecified)
        ;; (mini-echo-blue blue-warmer)
        ;; (mini-echo-yellow yellow)
        ;; (mini-echo-magenta magenta)
        ;; (mini-echo-blue blue-warmer)
        ;; (mini-echo-blue blue-warmer)
        ;; (transient-key-stay red)
        ;; (transient-key-exit green)
        ;; (transient-key-return yellow)
        ;; (transient-key-recurse magenta)
        ;; (transient-key-stack cyan)
        ))
;; (defun modus-vivendi-setup ()
;;   )

;; (add-hook 'modus-themes-after-load-theme-hook #'modus-vivendi-setup)
...
(use-package keyfreq
  ...
  (setq keyfreq-excluded-commands
        '(undefined ignore mwheel-scroll hippie-expand newline newline-and-indent
          ...
          pixel-scroll-precision python-indent-dedent-line-backspace))
  :config
...
(use-package quickrun
  ...
  (prependq! quickrun--language-alist
             '(("d2" . ((:command . "d2")
               ...
               ("python/uv" . ((:command . "uv")
                               (:exec . ("%c run %s"))
                               (:description . "Run python in uv project")))))
...
(use-package apheleia
  :hook ((typescript-ts-mode-hook js-ts-mode-hook python-ts-mode-hook bash-ts-mode-hook)
         . apheleia-mode)
  :init
  ...
  (alist-set! apheleia-mode-alist
              '((python-mode . (ruff ruff-isort))
                ...
                (bash-ts-mode . shfmt)
                ((json-ts-mode js-json-mode) . yq-json)

                ;; (ruby-ts-mode . rubocop)
                ;; ((html-mode html-ts-mode mhtml-mode mhtml-ts-mode nxml-mode) . biome)
                ;; ((css-mode css-ts-mode) . biome)
                ;; ((json-mode js-json-mode json-ts-mode) . biome)
                ;; ((js-mode js-ts-mode js-jsx-mode typescript-ts-mode tsx-ts-mode) . biome)
                )))
#+end_src
...
(use-package flymake
  ...
  :config
  ;; HACK remove ! before flymake overlay
  (put 'flymake-error 'before-string "")
  (put 'flymake-warning 'before-string "")
  (put 'flymake-note 'before-string ""))
...
(use-package flymake-collection
  ...
  (setq flymake-collection-hook-config
        '(((python-ts-mode python-mode) .
          ...
          (dotenv-mode flymake-collection-dotenv-linter)
...
#+begin_src emacs-lisp :lexical t :tangle no
(use-package eglot
  :hook ((bash-ts-mode-hook python-ts-mode-hook js-ts-mode-hook typescript-ts-mode-hook
                            css-mode-hook css-ts-mode-hook)
         . eglot-ensure)
  :init
  (setq eglot-autoshutdown t
        eglot-code-action-indications '(mode-line eldoc-hint)
  ...
  (setq-default eglot-workspace-configuration
                '(:ty
                  ...
                  (:enable t
                   ...
                   ;;              :completePropertyWithSemicolon t)
                   :validate t
                   ;; :hover (:documentation :json-false
                   ...
                   :lint (:compatibleVendorPrefixes "ignore"
                          ...
                          :propertyIgnoreDueToDisplay "warning"
                          :important "ignore"
                          :float "ignore"
                          :idSelector "ignore"
                          :unknownAtRules "warning")
                   :format (:enable t
                            :newlineBetweenSelectors t
                            :newlineBetweenRules t
                            :spaceAroundSelectorSeparator :json-false
                            :braceStyle "collapse"
                            :preserveNewLines t
  ...
  (defun my/eglot-multiserver-detect (interactive-p)
    "Detect and return multi server command when INTERACTIVE-P is nil."
    (unless interactive-p
      (let* ((project (project-root (eglot--current-project)))
             (emmet-p (and (executable-find "emmet-ls")
                           (list "--" "emmet-ls" "--stdio")))
             (tailwindcss-p
              (and (executable-find "tailwindcss-language-server")
                   (file-directory-p project)
                   (or (directory-files
                        project
                        nil
                        "tailwind\\.config\\.[m|c]?[j|t]s\\'")
                       (file-directory-p (expand-file-name "node_modules/tailwindcss" project)))
                   (list "--" "tailwindcss-language-server" "--stdio")))
             (eslint-p
              (and (executable-find "vscode-eslint-language-server")
                   (file-directory-p project)))
             server)
        (when (memq major-mode '(css-mode css-ts-mode))
          (-concat (list "rass" "--" "vscode-css-language-server" "--stdio") emmet-p tailwindcss-p)))))

  (prependq! eglot-server-programs
             `(((python-mode python-ts-mode) . ("rass" "python"))
               ;; ((html-mode html-ts-mode mhtml-mode css-mode css-ts-mode)
               ;;  . my/eglot-multiserver-detect)
               ))
...
(use-package lsp-bridge
  ...
  (lsp-bridge-mode-hook . lsp-bridge-breadcrumb-mode)
  :bind
  ...
  (setq lsp-bridge-breadcrumb-show-icon t)
  ...
  (setq lsp-bridge-default-mode-hooks (delq 'markdown-mode-hook lsp-bridge-default-mode-hooks)))
...
(use-package org
  ...
  (setq org-todo-keywords '((sequence "TODO(t)" "CACEL(c)" "DONE(d)" "|"))
        ...
        org-priority-faces '((?A . error)
                             ...
                             (?C . success)))
  ;; agenda
  (defun my/org-agenda-year-view (&optional year)
    "Advice to remove y-or-n."
    (interactive "P")
    (when year
      (setq year (org-small-year-to-year year)))
    (org-agenda-change-time-span 'year year))
  (advice-add 'org-agenda-year-view :override #'my/org-agenda-year-view)
  ...
  ;; tangle
  (defun my/org-babel-toggle-tangle ()
    "智能切换 :tangle 状态 (yes <-> no)。
1. 如果本地已有参数：在 yes/no 间切换；如果是文件名则报错。
2. 如果本地无参数：检查继承的全局属性。
   - 如果全局是 yes 或文件名 -> 插入 :tangle no (屏蔽全局)
   - 如果全局是 no 或 nil    -> 插入 :tangle yes (开启)"
    (interactive)
    (unless (org-in-src-block-p)
      (user-error "请在 Org 代码块 (Source Block) 内执行此命令"))

    (save-excursion
      ;; 1. 获取当前块的完整信息（包含继承的参数）
      ;; 'light 参数表示不进行繁重的代码求值，只解析 header
      (let* ((info (org-babel-get-src-block-info 'light))
             (params (nth 2 info))
             ;; 获取“实际生效”的 tangle 值 (可能是全局继承下来的)
             (effective-tangle (cdr (assq :tangle params)))
             ;; 准备用于本地匹配的 regex
        ...
        (if (re-search-forward regex limit t)
            ;; --- 情况 A: 本地显式定义了 :tangle ---
            (let ((local-val (match-string 1)))
              (cond
               ((string= local-val "yes")
                (replace-match "no" nil nil nil 1)
                (message "Tangle disabled (:tangle no)"))

               ((string= local-val "no")
                (replace-match "yes" nil nil nil 1)
                (message "Tangle enabled (:tangle yes)"))

               ;; 本地指定了文件名，报错保护
               (t
                (user-error "操作中止：当前 :tangle 指向了特定文件名 '%s'" local-val))))

          ;; --- 情况 B: 本地没有 :tangle (需要判断全局状态) ---
          (progn
            (end-of-line)
            (if (and effective-tangle
                     (not (string= effective-tangle "no")))
                ;; 如果全局是 "yes" 或者 "某文件名.py"
                ;; 我们的动作应该是：在本地插入 no 来“屏蔽”它
                (progn
                  (insert " :tangle no")
                  (message "Tangle disabled locally (Global was: %s)" effective-tangle))

              ;; 如果全局是 "no" 或者根本没设置 (nil)
              ;; 我们的动作应该是：在本地插入 yes 来“开启”它
              (progn
                (insert " :tangle yes")
                (message "Tangle enabled locally"))))))))
  :config
  (require 'org-capture)
  (require 'org-agenda)
